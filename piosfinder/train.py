'''
    ...
    Load the training and validation datasets and dataloaders
    Load the model and train and validate the model accordingly.

    
    Adapted from:
    classifier code from Benjamin Kellenberger from CV4Ecology 2022,
    and ResNet code in https://blog.paperspace.com/writing-resnet-from-scratch-in-pytorch/
'''
...
from dataset import NAIPDataset
...
def load_model(cfg):
    ...
    '''
    model_instance = ResNet(ResidualBlock, [3, 4, 6, 3])       # create an object instance of ResNet class
...
def train(cfg, dataLoader, model, optimizer):
    ...
    # running averages (loss and overall accuracy (oa))
...
def main():
    ...
    parser.add_argument('--config', help='Path to config file', default='configs/exp_piosfinder.yaml')
...
if __name__ == '__main__':
    ...
    # (i.e., "python piosfinder/train.py").
    main()
